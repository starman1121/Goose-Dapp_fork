...
import {BNB_TICKER, BUL_TICKER, ESHARE_TICKER, BTC_TICKER, ZAPPER_ROUTER_ADDR} from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  else if (zappingToken === BUL_TICKER) token = bombFinance.EGG;
  else if (zappingToken === ESHARE_TICKER) token = bombFinance.ESHARE;
