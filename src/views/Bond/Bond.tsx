...
const TITLE = 'goosefinance.xyz | Bonds'
...
const Bond: React.FC = () => {
  ...
  const bondBalance = useTokenBalance(bombFinance?.EBOND);
  ...
  const handleBuyBonds = useCallback(
    async (amount: string) => {
      ...
      addTransaction(tx, {
        summary: `Buy ${Number(amount).toFixed(2)} EBOND with ${amount} EGG`,
      });
    },
          ...
          <>
            ...
            <StyledBond>
              <StyledCardWrapper>
                <ExchangeCard
                  action="Purchase"
                  fromToken={bombFinance.EGG}
                  fromTokenName="EGG"
                  toToken={bombFinance.EBOND}
                  toTokenName="EBOND"
                  priceDesc={
                    !isBondPurchasable
                      ? 'EGG is over peg'
                      : getDisplayBalance(bondsPurchasable, 18, 2) + ' EBOND available for purchase'
                  }
              ...
              <StyledStatsWrapper>
                ...
                <ExchangeStat
                  tokenName="100 EBOND"
                  description="Current Price: (EGG)^2"
              ...
              <StyledCardWrapper>
                <ExchangeCard
                  action="Redeem"
                  fromToken={bombFinance.EBOND}
                  fromTokenName="EBOND"
                  toToken={bombFinance.EGG}
                  toTokenName="EGG"
                  priceDesc={`${getDisplayBalance(bondBalance)} EBOND Available in wallet`}
                  onExchange={handleRedeemBonds}
                  disabled={!bondStat || bondBalance.eq(0) || !isBondRedeemable}
                  disabledDescription={!isBondRedeemable ? `Enabled when 100 EGG > ${BOND_REDEEM_PRICE}BNB` : null}
                />
              </StyledCardWrapper>
            </StyledBond>
          </>
